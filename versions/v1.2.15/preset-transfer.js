...
export function createPresetTransferModule(deps) {
    ...
    let presetTransferLoadRevision = 0;
    ...
    function presetTransferRecords(document) {
        ...
        entries.forEach((raw, index) => {
            ...
            // Duplicate identifiers address the first entry only.
    ...
    async function savePendingPresetDocuments() {
        if (presetTransferSaving) return false;
        ...
        try {
            ...
            for (const [name, document] of [...presetTransferPendingDocuments.entries()]) {
                ...
                if (presetTransferPendingDocuments.get(name) !== document) continue;
            ...
            if (presetTransferPendingDocuments.size) {
                notify('本次修改已保存；期间新增的修改仍已暂存，请再次保存', 'info');
                return false;
            }
    ...
    async function loadPresetTransfer({ force = false } = {}) {
        ...
        const revision = ++presetTransferLoadRevision;
        ...
        try {
            ...
            const [source, target] = await Promise.all([
                loadWorkingPresetDocument(manager, presetTransferSource),
                presetTransferTarget ? loadWorkingPresetDocument(manager, presetTransferTarget) : null,
            ]);
            if (revision !== presetTransferLoadRevision) return;
            const sourceEntries = presetTransferRecords(source);
            const targetEntries = target ? presetTransferRecords(target) : [];
            presetTransferSourceEntries = sourceEntries;
            presetTransferTargetEntries = targetEntries;
            ...
            if (revision === presetTransferLoadRevision) presetTransferError = error.message || String(error);
        } finally {
            if (revision === presetTransferLoadRevision) {
    ...
    function clonePresetEntriesIntoDocument(targetDocument, targetInfo, selected, anchor, { renameCopies = false } = {}) {
        const records = presetTransferRecords(targetDocument);
        const existingIds = new Set(records.map((entry) => String(entry.id)));
        const existingNames = new Set(records.map((entry) => String(entry.name)));
        const clones = [];
    ...
    async function reorderPresetEntriesInternal(selected) {
        ...
        addPresetPromptOrders(document, selectedOrder, presetAnchorForOperation());
    ...
    function startPresetEntryEdit(id, side = 'source', surface = 'list') {
        ...
        presetTransferDraft = {
            ...
            enabled: entry.inserted && entry.enabled !== false,
            side: normalizedSide,
            presetName,
    ...
    async function savePresetEntryDraft() {
        ...
        if (group) {
            ...
            else if (draft.enabled !== false) addPresetPromptOrders(document, [{ identifier: draft.id, enabled: true }]);
        } else if (draft.enabled !== false) {
            addPresetPromptOrders(document, [{ identifier: draft.id, enabled: true }]);
        }
        verifyPresetOperation(document, [draft.id], { requireOrder: false });
        if (JSON.stringify(document) !== original) stagePresetDocument(presetName, document);
        presetTransferDraft = null;
    }
        ...
        const renderRows = (rows, side, { selectable = false } = {}) => {
            ...
            const body = rows.slice(0, presetTransferVisibleLimit).map((entry) => {
                ...
                return `<article class="ctb-preset-entry${checked ? ' is-selected' : ''}${entry.marker ? ' is-locked' : ''}${expanded ? ' is-expanded' : ''}">
                    <div class="ctb-preset-entry-head${selectable ? ' is-selectable' : ''}">${checkbox}<button type="button" class="ctb-preset-entry-title"${toggleAttrs}>${escapeHTML(entry.name)}</button><span class="ctb-preset-entry-status">${entry.inserted ? (entry.enabled ? '启用' : '停用') : '未加入主顺序'}</span><button type="button" class="ctb-preset-entry-chevron"${toggleAttrs} aria-label="${expanded ? '收起' : '展开'} ${escapeHTML(entry.name)}"><svg viewBox="0 0 16 16" focusable="false"><path d="M3.5 6 8 10.5 12.5 6"></path></svg></button></div>
