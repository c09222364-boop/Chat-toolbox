...
export function createIfBranchModule(deps) {
    ...
    function renderIfPrompts() {
        ...
        const rows = ifPrompts().map((prompt) => `<article class="ctb-if-prompt-card">
            <div class="ctb-if-card-head"><div><strong>${escapeHTML(prompt.name)}</strong>${prompt.tags ? `<small>${escapeHTML(prompt.tags)}</small>` : ''}</div><span>${countNonWhitespaceCharacters(prompt.content)} 字</span></div>
