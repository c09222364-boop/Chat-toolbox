...
function worldbookOriginalExtensionValue(raw, camelKey, snakeKey = camelKey, fallback = null) {
...
export function createWorldbookModule(deps) {
    ...
    async function saveWorldInfoDocument(name, data, { immediate = true, refreshList = false, verify: verifyAfterSave = false } = {}) {
        const context = getContext();
    ...
    async function saveCurrentWorldbook() {
        if (!worldbookBook || !worldbookDocument || worldbookSaving) return false;
        if (worldbookDraftDirty) applyWorldbookDraft({ quiet: true });
    ...
    async function renameWorldbookBook() {
        ...
        try {
            ...
            worldbookPendingDocuments.delete(oldName);
            worldbookBooks = [];
            await loadWorldbookManager({ force: true, book: name });
    ...
    function worldbookCompareDifferenceLabels(left, right) {
        ...
        if (Boolean(a.excludeRecursion) !== Boolean(b.excludeRecursion) || Boolean(a.preventRecursion) !== Boolean(b.preventRecursion) || worldbookOriginalExtensionValue(a, 'delayUntilRecursion', 'delay_until_recursion', false) !== worldbookOriginalExtensionValue(b, 'delayUntilRecursion', 'delay_until_recursion', false)) differences.push('递归设置');
        if (worldbookOriginalExtensionValue(a, 'useProbability', 'useProbability', true) !== worldbookOriginalExtensionValue(b, 'useProbability', 'useProbability', true) || Number(worldbookOriginalExtensionValue(a, 'probability', 'probability', 100)) !== Number(worldbookOriginalExtensionValue(b, 'probability', 'probability', 100))) differences.push('触发概率');
        if (worldbookOriginalExtensionValue(a, 'role', 'role', 0) !== worldbookOriginalExtensionValue(b, 'role', 'role', 0)) differences.push('消息角色');
        if (worldbookOriginalExtensionValue(a, 'scanDepth', 'scan_depth', null) !== worldbookOriginalExtensionValue(b, 'scanDepth', 'scan_depth', null)) differences.push('扫描深度');
        if (worldbookOriginalExtensionValue(a, 'caseSensitive', 'case_sensitive', null) !== worldbookOriginalExtensionValue(b, 'caseSensitive', 'case_sensitive', null) || worldbookOriginalExtensionValue(a, 'matchWholeWords', 'match_whole_words', null) !== worldbookOriginalExtensionValue(b, 'matchWholeWords', 'match_whole_words', null) || worldbookOriginalExtensionValue(a, 'useGroupScoring', 'use_group_scoring', null) !== worldbookOriginalExtensionValue(b, 'useGroupScoring', 'use_group_scoring', null)) differences.push('匹配规则');
        if (String(worldbookOriginalExtensionValue(a, 'group', 'group', '')) !== String(worldbookOriginalExtensionValue(b, 'group', 'group', '')) || Boolean(worldbookOriginalExtensionValue(a, 'groupOverride', 'group_override', false)) !== Boolean(worldbookOriginalExtensionValue(b, 'groupOverride', 'group_override', false)) || Number(worldbookOriginalExtensionValue(a, 'groupWeight', 'group_weight', 100)) !== Number(worldbookOriginalExtensionValue(b, 'groupWeight', 'group_weight', 100))) differences.push('分组设置');
        if (worldbookOriginalExtensionValue(a, 'sticky', 'sticky', null) !== worldbookOriginalExtensionValue(b, 'sticky', 'sticky', null) || worldbookOriginalExtensionValue(a, 'cooldown', 'cooldown', null) !== worldbookOriginalExtensionValue(b, 'cooldown', 'cooldown', null) || worldbookOriginalExtensionValue(a, 'delay', 'delay', null) !== worldbookOriginalExtensionValue(b, 'delay', 'delay', null)) differences.push('定时效果');
        if (String(worldbookOriginalExtensionValue(a, 'automationId', 'automation_id', '')) !== String(worldbookOriginalExtensionValue(b, 'automationId', 'automation_id', ''))) differences.push('自动化 ID');
        if (Boolean(worldbookOriginalExtensionValue(a, 'vectorized', 'vectorized', false)) !== Boolean(worldbookOriginalExtensionValue(b, 'vectorized', 'vectorized', false))) differences.push('向量化');
        if (String(worldbookOriginalExtensionValue(a, 'outletName', 'outlet_name', '')) !== String(worldbookOriginalExtensionValue(b, 'outletName', 'outlet_name', ''))) differences.push('命名出口');
        ...
        if (matchSourceKeys.some((key) => Boolean(worldbookOriginalExtensionValue(a, key, key.replace(/[A-Z]/g, (value) => `_${value.toLowerCase()}`), false)) !== Boolean(worldbookOriginalExtensionValue(b, key, key.replace(/[A-Z]/g, (value) => `_${value.toLowerCase()}`), false))) || !same(a.characterFilter || null, b.characterFilter || null)) differences.push('匹配范围');
        if (!same(worldbookOriginalExtensionValue(a, 'triggers', 'triggers', []), worldbookOriginalExtensionValue(b, 'triggers', 'triggers', []))) differences.push('生成触发器');
        if (Boolean(worldbookOriginalExtensionValue(a, 'ignoreBudget', 'ignore_budget', false)) !== Boolean(worldbookOriginalExtensionValue(b, 'ignoreBudget', 'ignore_budget', false))) differences.push('预算设置');
    ...
    async function simulateWorldbookTriggers() {
        ...
        const templateState = { api: null, env: null };
    ...
    function renderWorldbookAdvancedDifferences(raw, differences) {
        ...
        const probabilityEnabled = Boolean(worldbookOriginalExtensionValue(raw, 'useProbability', 'useProbability', true));
        add('触发概率', probabilityEnabled ? `${Number(worldbookOriginalExtensionValue(raw, 'probability', 'probability', 100))}%` : '不启用概率');
        const role = Number(worldbookOriginalExtensionValue(raw, 'role', 'role', 0));
        add('消息角色', ['system', 'user', 'assistant'][role] || String(role));
        const scanDepth = worldbookOriginalExtensionValue(raw, 'scanDepth', 'scan_depth', null);
        add('扫描深度', scanDepth === null || scanDepth === undefined ? '继承全局' : String(scanDepth));
        add('匹配规则', `大小写 ${onOffDefault(worldbookOriginalExtensionValue(raw, 'caseSensitive', 'case_sensitive', null))} · 全词 ${onOffDefault(worldbookOriginalExtensionValue(raw, 'matchWholeWords', 'match_whole_words', null))} · 分组评分 ${onOffDefault(worldbookOriginalExtensionValue(raw, 'useGroupScoring', 'use_group_scoring', null))}`);
        add('分组设置', `分组 ${String(worldbookOriginalExtensionValue(raw, 'group', 'group', '')) || '无'} · 覆盖 ${worldbookOriginalExtensionValue(raw, 'groupOverride', 'group_override', false) ? '开' : '关'} · 权重 ${Number(worldbookOriginalExtensionValue(raw, 'groupWeight', 'group_weight', 100))}`);
        add('定时效果', `粘性 ${worldbookOriginalExtensionValue(raw, 'sticky', 'sticky', null) ?? 0} · 冷却 ${worldbookOriginalExtensionValue(raw, 'cooldown', 'cooldown', null) ?? 0} · 延迟 ${worldbookOriginalExtensionValue(raw, 'delay', 'delay', null) ?? 0}`);
        add('自动化 ID', String(worldbookOriginalExtensionValue(raw, 'automationId', 'automation_id', '')) || '无');
        add('向量化', worldbookOriginalExtensionValue(raw, 'vectorized', 'vectorized', false) ? '开启' : '关闭');
        add('命名出口', String(worldbookOriginalExtensionValue(raw, 'outletName', 'outlet_name', '')) || '无');
        if (differences.includes('匹配范围')) {
            ...
            ].filter(([key, extensionKey]) => Boolean(worldbookOriginalExtensionValue(raw, key, extensionKey, false))).map(([, , label]) => label);
        ...
        const triggers = worldbookOriginalExtensionValue(raw, 'triggers', 'triggers', []);
        add('生成触发器', Array.isArray(triggers) && triggers.length ? triggers.map((item) => typeof item === 'string' ? item : JSON.stringify(item)).join('、') : '全部');
        add('预算设置', worldbookOriginalExtensionValue(raw, 'ignoreBudget', 'ignore_budget', false) ? '忽略预算' : '遵循预算');
    ...
    function renderWorldbookCompareSide(record, side, highlightedContent, differences) {
        ...
        return `<div class="ctb-preset-compare-side ctb-worldbook-compare-side">
            ...
            <div class="ctb-worldbook-compare-meta"><span${differenceClass('状态')}>${escapeHTML(status)}</span><span${differenceClass('位置')}>${escapeHTML(position)}</span><span${differenceClass('深度')}>深度 ${worldbookDepth(raw)}</span><span${differenceClass('优先级')}>优先级 ${worldbookOrder(raw)}</span><span${differenceClass('关键词')}>主关键词：${escapeHTML(primary)}</span><span${differenceClass('关键词')}>次要关键词：${escapeHTML(secondary)}</span>${differences.includes('递归设置') ? `<span class="is-different">递归：不可递归 ${raw.excludeRecursion ? '开' : '关'} · 阻止进一步递归 ${raw.preventRecursion ? '开' : '关'} · 仅递归时检查 ${worldbookOriginalExtensionValue(raw, 'delayUntilRecursion', 'delay_until_recursion', false) ? '开' : '关'}</span>` : ''}${renderWorldbookAdvancedDifferences(raw, differences)}</div>
    ...
    async function handleAction(action, data = {}) {
        switch (action) {
            case 'refresh-worldbook': stageWorldbookDraft(); await loadWorldbookManager({ force: true, book: worldbookBook }); return true;
