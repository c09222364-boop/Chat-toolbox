...
(function () {
    ...
    function syncTransientNotice() {
        ...
        const existing = root.querySelector('.ctb-notice-overlay[role="alertdialog"]');
        if (!transientNotice) {
            existing?.remove();
            return;
        }
    ...
    function restorePanelScroll(tab = activeTab) {
        ...
        if (!root) return;
        const card = root.querySelector('.ctb-card');
        const apply = () => {
            if (root.hidden || activeTab !== tab || root.querySelector('.ctb-card') !== card) return;
            const body = root.querySelector('.ctb-body');
            const tabs = root.querySelector('.ctb-tabs');
