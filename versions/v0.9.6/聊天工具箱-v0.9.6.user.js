...
(function () {
        ...
        try {
            ...
            const next = {
                ...
                presetTransfer: {
                    ...
                    loadMode: stored.presetTransfer?.loadMode === 'enabled' ? 'enabled' : 'all',
                },
            };
    ...
    function apiOutputText(json) {
        const standard = typeof json === 'string'
            ? json
    ...
    async function callAiText(feature, messages) {
        ...
        if (!channel) {
            ...
            const output = await generateRaw.call(context, { prompt: messages, responseLength: 65535 });
    ...
    function renderWorldbookRowsV2(records) {
        return records.map((record) => {
            ...
            // The light button represents the preferred light type even when
            // the entry itself is disabled.  Keep that visual state separate
            // from the row/power-off state so the two controls remain
            // independent and the selected blue/green setting stays visible.
    ...
    function presetTransferLoadMode() {
        return settings?.presetTransfer?.loadMode === 'enabled' ? 'enabled' : 'all';
    }
    ...
    function createUI() {
        ...
        style.textContent = `
            ...
            #${ROOT_ID} .ctb-worldbook-entry.is-off{border-left-color:#94a3b8;background:#f8fafc;}
            ...
            #${ROOT_ID} .ctb-preset-placement-fields{min-width:0;}
            #${ROOT_ID} .ctb-preset-placement-fields .ctb-input:first-child{flex:0 0 150px;}
            #${ROOT_ID} .ctb-preset-placement-fields .ctb-input:last-child{flex:1;}
            ...
            @media (max-width:560px){
                ...
                #${ROOT_ID} .ctb-preset-placement-fields{flex-wrap:wrap;}
                #${ROOT_ID} .ctb-preset-placement-fields .ctb-input:first-child{flex:1 1 100%;}
            }
            ...
            #${ROOT_ID} .ctb-toggle input:checked+span{background:var(--ctb-accent)!important;}
    ...
    async function handleChange(event) {
        ...
        if (id === 'ctb-regex') ui.regex = target.checked;
            ...
            settings.presetTransfer.loadMode = target.value === 'enabled' ? 'enabled' : 'all';
            presetTransferSelected = new Set();
            presetTransferAnchor = { kind: 'top', anchorId: '' };
            presetTransferDraft = null;
            presetTransferVisibleLimit = 120;
