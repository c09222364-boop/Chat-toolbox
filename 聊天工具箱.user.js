...
(function () {
    ...
    function loadSettings() {
        try {
            ...
            next.ifBranch.prompts = (Array.isArray(stored.ifBranch?.prompts) ? stored.ifBranch.prompts : [])
                .map(normalizeIfPrompt)
                .filter(Boolean);
            next.ifBranch.favorites = (Array.isArray(stored.ifBranch?.favorites) ? stored.ifBranch.favorites : [])
    ...
    function ifPrompts() {
        return Array.isArray(settings.ifBranch?.prompts) ? settings.ifBranch.prompts : [];
    }

    function ifFavorites() {
        return Array.isArray(settings.ifBranch?.favorites) ? settings.ifBranch.favorites : [];
    }
    ...
    function deleteIfPrompt(id) {
        ...
        settings.ifBranch.prompts = ifPrompts().filter((item) => item.id !== prompt.id);
    ...
    function markLatestIfBranchMessage(messageRef) {
        ...
        host.setTimeout(() => {
            if (root && !root.hidden && activeTab === 'if-branch') renderPanel();
        }, 120);
    }
    ...
    function bindIfBranchEvents() {
        ...
        bind(types.MESSAGE_DELETED, () => {
            if (root && !root.hidden && activeTab === 'if-branch') host.setTimeout(renderPanel, 80);
        });
    ...
    async function finishIfBranch(mode) {
        ...
        try {
            if (mode === 'keep') {
                ...
                await saveChat();
                if (favorite) {
                    settings.ifBranch.favorites.unshift(favorite);
                    saveSettings();
                }
            }
            ifBranchReader = null;
    ...
    function deleteIfFavorite(id) {
        ...
        settings.ifBranch.favorites = ifFavorites().filter((item) => item.id !== favorite.id);
    ...
    function handleGlobalKeydown(event) {
        if (event.key !== 'Escape' || !ifBranchReader || destroyed) return;
    ...
    function createUI() {
        ...
        style.textContent = `
            ...
            #${ROOT_ID} .ctb-primary-soft{border-color:#91aa98;background:#e0e9e3;color:#4d6755;}
            #${ROOT_ID} .ctb-world-empty{display:flex;align-items:center;justify-content:center;gap:7px;min-height:70px;padding:10px;color:#858a87;font-size:10px;} #${ROOT_ID} .ctb-world-error{color:#985957;}
            ...
            #${ROOT_ID} .ctb-if-intro{margin:0;color:#5e646b;font-size:10px;line-height:1.55;} #${ROOT_ID} .ctb-if-tabs{margin:7px 0;} #${ROOT_ID} .ctb-if-list{display:grid;gap:7px;} #${ROOT_ID} .ctb-if-prompt-card,#${ROOT_ID} .ctb-if-favorite-card{padding:8px;border:1px solid #cfd3d8;border-radius:5px;background:#eceff2;} #${ROOT_ID} .ctb-if-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;} #${ROOT_ID} .ctb-if-card-head>div{display:grid;min-width:0;gap:2px;} #${ROOT_ID} .ctb-if-card-head strong{overflow:hidden;color:#3f454c;font-size:11px;text-overflow:ellipsis;white-space:nowrap;} #${ROOT_ID} .ctb-if-card-head small{overflow:hidden;color:#78808a;font-size:9px;text-overflow:ellipsis;white-space:nowrap;} #${ROOT_ID} .ctb-if-card-head>span{flex:0 0 auto;color:#6d7580;font-size:9px;white-space:nowrap;} #${ROOT_ID} .ctb-if-prompt-card>p{display:-webkit-box;max-height:62px;margin:6px 0;overflow:hidden;color:#555c64;font-size:10px;line-height:1.5;-webkit-box-orient:vertical;-webkit-line-clamp:3;white-space:pre-wrap;word-break:break-word;} #${ROOT_ID} .ctb-if-card-actions{justify-content:flex-end;flex-wrap:wrap;} #${ROOT_ID} .ctb-if-tags{margin:6px 0;color:#68717c;font-size:9px;} #${ROOT_ID} .ctb-if-editor{display:grid;gap:7px;} #${ROOT_ID} .ctb-if-editor .ctb-field{display:grid;gap:4px;color:#4d555d;font-size:10px;font-weight:600;} #${ROOT_ID} .ctb-if-prompt-content{min-height:150px;height:150px;} #${ROOT_ID} .ctb-if-editor-actions{justify-content:flex-end;} #${ROOT_ID} .ctb-if-active{border-color:#9eacb9;background:#e6ecf1;} #${ROOT_ID} .ctb-if-active.is-error{border-color:#c99a9a;background:#f3e7e7;} #${ROOT_ID} .ctb-if-active-summary{display:grid;gap:3px;} #${ROOT_ID} .ctb-if-active-summary strong{color:#39434d;font-size:12px;} #${ROOT_ID} .ctb-if-active-summary small{color:#727b85;font-size:9px;} #${ROOT_ID} .ctb-if-active-summary p{margin:3px 0 0;color:#56616c;font-size:10px;line-height:1.5;} #${ROOT_ID} .ctb-if-finish-actions{display:flex;justify-content:flex-end;flex-wrap:wrap;gap:6px;margin-top:8px;} #${ROOT_ID} .ctb-reader-overlay{position:fixed;inset:0;z-index:2147483150;display:grid;place-items:center;padding:10px;background:rgba(18,21,27,.5);} #${ROOT_ID} .ctb-reader-card{width:min(1100px,calc(100vw - 20px));height:calc(100vh - 20px);height:calc(100dvh - 20px);max-height:900px;display:flex;flex-direction:column;overflow:hidden;border:1px solid #bfc3c8;border-radius:4px;background:#f5f5f7;box-shadow:0 18px 48px rgba(0,0,0,.38);} #${ROOT_ID} .ctb-reader-header{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 12px;border-bottom:1px solid #d3d5d9;background:#fafafd;color:#4c5056;font-size:12px;font-weight:700;} #${ROOT_ID} .ctb-reader-content{flex:1;min-height:0;overflow:auto;padding:15px;background:#f0f0f2;color:#42464c;font:12px/1.65 var(--mainFontFamily,Arial,sans-serif);word-break:break-word;} #${ROOT_ID} .ctb-if-reader-content{display:grid;align-content:start;gap:9px;} #${ROOT_ID} .ctb-if-reader-message{padding:9px;border:1px solid #d3d8dd;border-radius:5px;background:#fff;} #${ROOT_ID} .ctb-if-reader-message.is-user{border-color:#b9cbd9;background:#f2f7fa;} #${ROOT_ID} .ctb-if-reader-message>div{display:flex;justify-content:space-between;gap:8px;margin-bottom:6px;color:#68727d;font-size:10px;} #${ROOT_ID} .ctb-if-reader-message>div strong{color:#414a53;} #${ROOT_ID} .ctb-if-reader-message pre{margin:0;white-space:pre-wrap;word-break:break-word;font:inherit;}
        ...
        root.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && ifBranchReader) {
                event.preventDefault();
                ifBranchReader = null;
                renderPanel();
                return;
    ...
    function handleInput(event) {
        ...
        else if (id === 'ctb-if-prompt-name' && ifPromptEditor) { ifPromptEditor.draft.name = target.value; return; }
        else if (id === 'ctb-if-prompt-tags' && ifPromptEditor) { ifPromptEditor.draft.tags = target.value; return; }
        else if (id === 'ctb-if-prompt-content' && ifPromptEditor) { ifPromptEditor.draft.content = target.value; return; }
    ...
    async function handleChange(event) {
        ...
        if (!target) return;
        const id = target.id;
        if (id === 'ctb-post-edit-system' || id === 'ctb-post-edit-rules') flushSettingsSave();
    ...
    async function handleAction(action, data) {
        switch (action) {
            ...
            case 'delete-channel': {
                ...
                if (index < 0 || !host.confirm('确定删除这个生成渠道吗？使用它的词句修改会改为跟随酒馆主接口。')) return;
            ...
            case 'set-if-library-view': ifBranchLibraryView = data.ifView === 'favorites' ? 'favorites' : 'prompts'; ifPromptEditor = null; return renderPanel();
            case 'new-if-prompt': return beginNewIfPrompt();
            case 'edit-if-prompt': return beginEditIfPrompt(data.ifPromptId);
            case 'save-if-prompt': return saveIfPromptEditor();
            case 'cancel-if-prompt': return cancelIfPromptEditor();
            case 'delete-if-prompt': return deleteIfPrompt(data.ifPromptId);
            case 'fill-if-prompt': return fillIfPrompt(data.ifPromptId);
            case 'start-if-branch': return startIfBranch(data.ifPromptId);
            case 'finish-if-collect': return finishIfBranch('collect');
            case 'finish-if-discard': return finishIfBranch('discard');
            case 'finish-if-keep': return finishIfBranch('keep');
            case 'clear-broken-if-branch': return clearBrokenIfBranchState();
            case 'open-if-favorite': ifBranchReader = { favoriteId: data.ifFavoriteId }; return renderPanel();
            case 'copy-if-favorite': return copyIfFavorite(data.ifFavoriteId);
            case 'export-if-favorite': return exportIfFavorite(data.ifFavoriteId);
            case 'rename-if-favorite': return renameIfFavorite(data.ifFavoriteId);
            case 'delete-if-favorite': return deleteIfFavorite(data.ifFavoriteId);
            case 'close-if-reader': ifBranchReader = null; return renderPanel();
